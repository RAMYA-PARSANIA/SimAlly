/*
  ...
  1. Functions
    - `store_encrypted_gmail_tokens_with_fallback` - Securely store Gmail tokens with encryption
    - `get_decrypted_gmail_tokens_with_fallback` - Retrieve and decrypt tokens with fallback handling
    - `check_gmail_tokens_exist` - Check if tokens exist for a user
    - `revoke_gmail_tokens` - Revoke and delete Gmail tokens
  ...
  2. Security
    ...
    - Tokens are encrypted using base64 encoding (simplified for this implementation)
...
DROP FUNCTION IF EXISTS store_encrypted_gmail_tokens_with_fallback(UUID, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT);
DROP FUNCTION IF EXISTS get_decrypted_gmail_tokens_with_fallback(UUID, TEXT);
DROP FUNCTION IF EXISTS check_gmail_tokens_exist(UUID);
DROP FUNCTION IF EXISTS revoke_gmail_tokens(UUID);
DROP FUNCTION IF EXISTS update_gmail_tokens_updated_at();
