/*
  ...
  1. New Functions
    ...
    - `get_decrypted_gmail_tokens_with_fallback` - Retrieve and decrypt Gmail tokens with fallback
    - `check_gmail_tokens_exist` - Check if tokens exist for a user
    - `revoke_gmail_tokens` - Revoke and delete Gmail tokens
    - `update_gmail_tokens_updated_at` - Update timestamp trigger function

  2. Security
    - All functions use proper encryption/decryption
    - Session-based security with fallback mechanisms
