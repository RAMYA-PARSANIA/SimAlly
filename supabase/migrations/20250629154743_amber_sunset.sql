/*
  ...
  1. Updates
    - Add notification_check_interval to notification_settings
    - Add task_due_notification_hours to notification_settings
    - Add event_reminder_notification_minutes to notification_settings
    - Add function to check for upcoming tasks, events, and reminders
    - Add trigger to automatically create notification settings for new users
...
-- Add notification check interval to notification_settings
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Add task due notification hours to notification_settings
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Add event reminder notification minutes to notification_settings
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Create function to check for upcoming tasks, events, and reminders
CREATE OR REPLACE FUNCTION check_upcoming_items() RETURNS VOID AS $$
DECLARE
  user_record RECORD;
  task_record RECORD;
  event_record RECORD;
...
BEGIN
  ...
  FOR user_record IN 
    SELECT ns.user_id, ns.task_due_notification_hours, ns.event_reminder_notification_minutes
    FROM notification_settings ns
    WHERE ns.task_reminders = true OR ns.event_reminders = true OR ns.reminder_notifications = true
  LOOP
    notification_hours := COALESCE(user_record.task_due_notification_hours, 24);
    
    -- Check for tasks due soon
    IF user_record.task_reminders THEN
      FOR task_record IN
    ...
    IF user_record.event_reminders THEN
      FOR event_record IN
        ...
        WHERE ce.user_id = user_record.user_id
          ...
          AND ce.start_time <= (now_time + (user_record.event_reminder_notification_minutes || ' minutes')::INTERVAL)
          AND NOT EXISTS (
            SELECT 1 FROM notifications n
            WHERE n.user_id = user_record.user_id
    ...
    -- Check for reminders
    IF user_record.reminder_notifications THEN
