/*
  ...
  3. Changes
    - Removed problematic default channel creation
    - Fixed user registration function to create default channels
*/
...
BEGIN
  ...
  -- Check if default channels exist, if not create them
  SELECT id INTO general_channel_id FROM public.channels WHERE name = 'General' AND type = 'public' LIMIT 1;
  ...
  IF general_channel_id IS NULL THEN
    ...
    VALUES (general_channel_id, new.id, 'admin');
  ELSE
  ...
  -- Check if Random channel exists
  SELECT id INTO random_channel_id FROM public.channels WHERE name = 'Random' AND type = 'public' LIMIT 1;
  
  IF random_channel_id IS NULL THEN
    -- Create Random channel
    INSERT INTO public.channels (name, description, type, created_by)
    VALUES ('Random', 'Random conversations and fun', 'public', new.id)
    RETURNING id INTO random_channel_id;
    
    -- Add creator as admin
    INSERT INTO public.channel_members (channel_id, user_id, role)
    VALUES (random_channel_id, new.id, 'admin');
  ELSE
    -- Add user to existing Random channel
    INSERT INTO public.channel_members (channel_id, user_id)
    VALUES (random_channel_id, new.id)
    ON CONFLICT DO NOTHING;
  END IF;
