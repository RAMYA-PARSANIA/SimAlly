...
DROP FUNCTION IF EXISTS store_encrypted_gmail_tokens(uuid, text, text, text, text, timestamptz, text);
DROP FUNCTION IF EXISTS get_decrypted_gmail_tokens(uuid, text);
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE user_encryption_keys ADD COLUMN key_salt text NOT NULL DEFAULT gen_salt('bf', 12);
  END IF;
END $$;

-- Create the store_encrypted_gmail_tokens function with REAL encryption
...
-- Create the get_decrypted_gmail_tokens function with REAL decryption
...
-- Function to cleanup expired tokens and keys (with correct return type)
